    ...
    public DbIterator physicalPlan(TransactionId t, Map<String,TableStats> baseTableStats, boolean explain) throws ParsingException {
        ...
        while (filterIt.hasNext()) {
            ...
            try {//td.fieldNameToIndex(disambiguateName(lf.fieldPureName))
                ftyp = td.getType(td.nameToId(lf.fieldQuantifiedName));
            ...
            Predicate p = null;
            try {
                p = new Predicate(subplan.getTupleDesc().nameToId(lf.fieldQuantifiedName), lf.p,f);
            } catch (NoSuchElementException e) {
            ...
            double sel= s.estimateSelectivity(subplan.getTupleDesc().nameToId(lf.fieldQuantifiedName), lf.p, f);
...
//                    id = 
        ...
        if (hasOrderBy) {
            node = new OrderBy(node.getTupleDesc().nameToId(oByField), oByAsc, node);
        }
